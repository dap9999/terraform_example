...
resource "aws_instance" "aws07_instance" {
  ami                         = var.ami_id
...
resource "aws_ami_from_instance" "aws07_ami" {
  name                    = "${var.prefix}-instance-ami"
  source_instance_id      = aws_instance.aws07_instance.id
  snapshot_without_reboot = true
  depends_on              = [null_resource.aws07_delay]
  tags = {
    Name = "${var.prefix}-instance-ami"
  }
}
